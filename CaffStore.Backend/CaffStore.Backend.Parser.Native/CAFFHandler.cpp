...
namespace CAFF {
    ...
    Credits CAFFHandler::handleCredits(std::vector<unsigned char>& buffer, Block& block) {
    ...
    Animation CAFFHandler::handleAnimation(std::vector<unsigned char>& buffer) {
        ...
        CIFFFile ciff = ciffHandler.parseCIFF(buffer);
    ...
    Header CAFFHandler::handleHeader(std::vector<unsigned char>& buffer, Block& block) {
        Header header;
        char magic[4];
        Converter::BytesToIntConverter bytesToIntConverter;
    ...
    CAFF:CAFFFile caffFile;
        ...
        caffFile.blocks = new Block[blocks.size()];
 ...
 static CAFFFile handleParsing(const char* filename){
    ...
    char* parseToJson(BYTE* pArray, int nSize, unsigned char** data, int* size, bool* isError) {
        ...
        try {
            ...
            while (buffer.size() > 0) {
                ...
                std::cout << std::endl << "Handling block ..." << std::endl;
                int length = bytesToIntConverter.convert8BytesToInteger(buffer);
                InteropHeader interHeader;
                InteropCredits interCredits;
                Block block;
                block.id = identifier;
                block.length = length;
        ...
        *data = static_cast<uint8_t*>(malloc(sizePrev));
        ...
        int r = strcpy_s(pszReturn, ulSize, array);
        // Return pszReturn.
        return pszReturn;
